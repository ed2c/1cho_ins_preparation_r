...
Inschrijvingen <- Inschrijvingen %>%
  mutate(
    ...
    INS_Opleidingsfase_actueel = INS_Opleidingsfase_BPM
  )
...
Inschrijvingen <- Inschrijvingen %>%
  ...
  group_by(
    INS_Studentnummer,
    INS_Opleidingsnaam_Z08 # , INS_Instelling
  ) %>%
  ## Corrigeer de INS_Eerste_jaar_opleiding_en_instellingen voor studenten die
  ## niet in 1cho zitten
  mutate(
    INS_Eerste_jaar_opleiding_en_instelling = ifelse(
      is.na(INS_Eerste_jaar_opleiding_en_instelling),
      INS_Inschrijvingsjaar,
      INS_Eerste_jaar_opleiding_en_instelling
    ),
    INS_Eerste_jaar_opleiding_en_instelling = min(INS_Eerste_jaar_opleiding_en_instelling,
                                                  na.rm = TRUE
    )
  ) %>%
  ...
  arrange(
    INS_Studentnummer,
    INS_Opleidingsnaam_Z08, INS_Inschrijvingsjaar
  ) %>%
  mutate(
    ## Maak een logische variabele aan om aan te geven of het
    ## inschrijvingsjaar het EOI-jaar is
    ...
    # Studiejaar kan alleen berekend worden als EOI bekend is in onze data,
    # maak hiervoor een variabele aan.
    INS_EOI_in_data = case_when(
      any(INS_Inschrijvingsjaar_is_EOI) ~ TRUE
    )
  ) %>%
  # Bereken INS_Studiejaar op basis van rijnummer, het bestand is gegroepeerd
  # en gesorteerd, dus de eerste rij is het eerste studiejaar.
  mutate(
    INS_Studiejaar = if_else(
      INS_EOI_in_data, as.double(row_number()), NA_real_
    ),
    # INS_Inschrijvingsjaar_EOI wordt hier gekopieerd van
    # INS_Eerste_jaar_opleiding_en_instelling
    INS_Inschrijvingsjaar_EOI = INS_Eerste_jaar_opleiding_en_instelling,
    # INS_Tussenjaar_binnen_opleiding berekenen
    INS_Tussenjaar_binnen_opleiding = max(
      INS_Inschrijvingsjaar
    ) - min(
      INS_Inschrijvingsjaar
    ) - (max(
    ...
    ## Als er sinds de laatste 1cho nog niet eerder ingeschreven studenten zijn
    ## bijgekomen geeft deze dan Studiejaar
    # INS_Studiejaar = ifelse(INS_Inschrijvingsjaar %in% c(2019, 2020) & is.na(INS_Studiejaar), 1, INS_Studiejaar)
  ) %>%
  select(
    -INS_Inschrijvingsjaar_is_EOI,
    -INS_EOI_in_data
  ) %>%
  ungroup() # Groepering weer ongedaan maken
